        ...
        if (response.ok) {
            ...
            if (result.error_code === 'TOKEN_EXPIRED_OR_REVOKED') {
                ...
                if (clientId) {
                    ...
                    // Store a flag in localStorage to show a message after re-authentication
                    localStorage.setItem('google_reauth_required', 'true');
                    
                    // Redirect to Google OAuth
                    window.location.href = `https://accounts.google.com/o/oauth2/v2/auth?${params.toString()}`;
                    return null; // Exit early since we're redirecting
                } else {
...
export const calendarService = {
  ...
  async processCalendarEvent(event: GoogleCalendarEvent, userId: string, credentials: GoogleCredentials): Promise<{ action: string; meetingId?: string }> {
    ...
    } else if (event.creator && event.creator.email === credentials.googleEmail) {
