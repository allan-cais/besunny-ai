#!/usr/bin/env python3
"""
Test app v12 - Testing enhanced config module
This app tests the enhanced config module with real environment variable handling
"""
...
class Settings:
    ...
    def __init__(self):
        self.app_name: str = "BeSunny.ai Backend v12"
...
# Test enhanced config module
def test_enhanced_config():
    """Test the enhanced config module with real environment variables."""
    logger.info("Testing enhanced config module...")
    ...
    try:
        ...
        logger.info("Testing enhanced app.core.config import...")
        from app.core.config import (
            get_settings as real_get_settings,
            is_development as real_is_development,
            is_production,
            is_staging,
            is_testing,
            get_database_url,
        ...
        logger.info("✅ Enhanced app.core.config import successful")
        
        # Test the enhanced config
        try:
            real_settings = real_get_settings()
            logger.info(f"✅ Enhanced config loaded: {real_settings.app_name}")
            logger.info(f"✅ Environment: {real_settings.environment}")
            logger.info(f"✅ Database URL: {real_settings.database.database_url}")
            logger.info(f"✅ Redis URL: {real_settings.redis.redis_url}")
            logger.info(f"✅ CORS Origins: {real_settings.cors.cors_origins}")
            logger.info(f"✅ Log Level: {real_settings.logging.log_level}")
            
            # Test environment functions
            logger.info(f"✅ is_development(): {real_is_development()}")
            logger.info(f"✅ is_production(): {is_production()}")
            logger.info(f"✅ is_staging(): {is_staging()}")
            logger.info(f"✅ is_testing(): {is_testing()}")
            
            # Test utility functions
            logger.info(f"✅ get_database_url(): {get_database_url()}")
            logger.info(f"✅ get_redis_url(): {get_redis_url()}")
            logger.info(f"✅ get_cors_origins(): {get_cors_origins()}")
            logger.info(f"✅ get_log_level(): {get_log_level()}")
        ...
        logger.error(f"❌ Enhanced app.core.config import failed: {e}")
        return False
    
    try:
        logger.info("Testing app.core.database import...")
        from app.core.database import init_db, close_db
        logger.info("✅ app.core.database import successful")
    except Exception as e:
        logger.error(f"❌ app.core.database import failed: {e}")
        return False
...
async def lifespan(app: FastAPI):
    """Application lifespan manager - testing enhanced config module."""
    # Startup
    logger.info("Starting BeSunny.ai Python Backend v12")
    
    try:
        # Test enhanced config first
        config_success = test_enhanced_config()
        if not config_success:
            logger.warning("Enhanced config test failed, continuing with fallback services")
        ...
        # Shutdown
        logger.info("Shutting down BeSunny.ai Python Backend v12")
    ...
    async def health_check():
        ...
        try:
            return {
                ...
                "timestamp": time.time(),
                "message": "Backend is running successfully with enhanced config module"
            }
        except Exception as e:
        ...
        try:
            if static_dir.exists():
                ...
                return {
                    ...
                    "note": "Enhanced config module loaded - testing production-ready configuration"
                }
        except Exception as e:
            return {
                "message": f"Welcome to {settings.app_name}",
...
if __name__ == "__main__":
    ...
    print(f"🔌 Mode: Testing enhanced config module with production-ready settings")
