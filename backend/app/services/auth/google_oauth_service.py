...
class GoogleOAuthService:
    ...
    async def handle_oauth_callback(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🔍 OAuth Debug - Tokens received successfully - access_token_length: {len(tokens.accessToken) if tokens.accessToken else 0}")
    ...
    async def handle_workspace_oauth_callback(self, code: str, user_id: str, redirect_uri: str, supabase_access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🔍 OAuth Debug - OAuth Service: Tokens received - access_token_length: {len(tokens.accessToken) if tokens.accessToken else 0}, expires_in: {tokens.expires_in}")
    ...
    async def _store_google_credentials(self, user_id: str, tokens: OAuthTokens, user_info: UserInfo) -> bool:
        ...
        try:
            ...
            logger.info(f"🔍 OAuth Debug - Access token length: {len(tokens.accessToken) if tokens.accessToken else 0}")
            logger.info(f"🔍 OAuth Debug - Refresh token length: {len(tokens.refreshToken) if tokens.refreshToken else 0}")
